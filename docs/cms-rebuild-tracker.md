...
- `done` Firebase Storage 제한을 확인한 뒤, CMS 자산 업로드를 로컬 자산 저장 + 출간 시 GitHub 반영 구조로 전환했다.
- `done` 업로드한 자산은 편집 중 로컬 `public/uploads`에서 미리보기하고, 출간 시 실제로 참조된 자산만 GitHub에 반영하도록 정리했다.
- `done` 출간 시 GitHub 반영과 함께 로컬 `content/posts`도 갱신해 localhost `/blog`에서 새 글과 삭제 결과를 바로 확인할 수 있게 맞췄다.
...
- 2026-03-26: `publish-post`와 `delete-post` API가 로컬 `content/posts`도 함께 갱신하도록 바꿔, localhost `/blog`에서 새 글과 삭제 결과를 바로 확인할 수 있게 맞췄다.
- 2026-03-26: 자산 업로드만으로는 GitHub 커밋이 생기지 않게 바꾸고, 실제 GitHub 자산 반영은 출간 시점에만 일어나도록 정리했다.
