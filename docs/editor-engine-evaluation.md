...
- 실사용 감각 쪽도 추가 보강했다.
  - 대시보드의 출간 시스템 상태는 긴 텍스트를 촘촘한 작은 카드에 몰아넣지 않고, 상태 체크와 출간 대상을 분리한 읽기 쉬운 레이아웃으로 다시 짰다.
  - 포스트 목록의 출간 완료 배너도 “어디를 눌러야 하는지”와 “다음에 무엇을 확인해야 하는지”가 더 명확히 보이게 정리했다.
  - CodeMirror는 markdown 하이라이트 대비를 더 올려서 제목, 링크, 강조, 코드, 구분선이 평문과 덜 섞여 보이게 맞췄다.
  - 수동 임시저장에는 즉시 피드백 배지를 붙여 버튼을 눌렀을 때 아무 반응이 없는 느낌을 줄였다.
  - 이후 사용자 피드백을 반영해, 출간 시스템 상태의 `상태 체크`는 작은 멀티컬럼 카드 대신 세로 리스트형 카드로 다시 단순화했다.
  - Storage 업로드는 bucket 후보를 순차 재시도하도록 바꿨고, 첫 후보에서 `storage/unknown`이 나와도 두 번째 후보를 실제로 시도하게 했다.
