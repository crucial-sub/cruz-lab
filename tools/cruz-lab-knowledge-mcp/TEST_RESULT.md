...
- 핵심 내용:
  ...
  - 관련 포스트 수는 `stock-lab: 2`, `cruz-lab: 2`, `jungle-tetris: 2`, `llm-yatcha: 0`으로 반환됐다.
...
- 결과: 성공
- 핵심 내용: `data/final-posts/README.md`, `data/final-posts/original_resume.md`, `data/final-posts/projects/cruz-lab-v2-offline-accessibility.md`, `data/resume/jungsub_resume.md`, `src/content/project-posts/cruz-lab-operating-cms.md`, `src/content/projects/cruz-lab.md` 등이 반환됐다.

- 입력: `{ query: "SSE", limit: 10 }`
- 결과: 성공
- 핵심 내용: `original_resume.md`, `llm-yatcha` 관련 final post, `stock-lab` 관련 final/project posts, `jungsub_resume.md`, `src/content/project-posts/stock-lab-realtime-streaming.md`, `src/content/projects/cruz-lab.md` 등이 반환됐다.

- 입력: `{ query: "Web Worker", limit: 10 }`
- 결과: 성공
- 핵심 내용: `original_resume.md`, `data/final-posts/projects/stock-lab-part2-realtime-cost.md`, `data/resume/jungsub_resume.md`, `src/content/project-posts/stock-lab-realtime-cost.md`가 반환됐다.
...
- 입력: `{ pathOrSlug: "src/content/projects/cruz-lab.md" }`
- 결과: 성공
- 핵심 내용: `Cruz Lab` 프로젝트 문서를 읽었다. frontmatter의 `title`, `description`, `tags`, `tech`, `github`, `featured`, `order`와 본문이 포함됐다.

- 입력: `{ pathOrSlug: "src/content/project-posts/cruz-lab-operating-cms.md" }`
- 결과: 성공
...
- 결과: 성공
- 핵심 내용:
  - `src/content/projects/cruz-lab.md` 프로젝트 본문이 포함됐다.
  - 관련 project-post로 `cruz-lab-operating-cms`, `cruz-lab-v2-offline-accessibility` 2건이 함께 묶였다.

- 입력: `{ slug: "stock-lab" }`
- 결과: 성공
- 핵심 내용:
  - `src/content/projects/stock-lab.md` 프로젝트 본문이 포함됐다.
...
- 결과: 성공
- 핵심 내용: `get_project_context`의 `cruz-lab` 결과와 같은 구조로, 프로젝트 본문과 관련 project-post 2건이 JSON으로 반환됐다.
...
- 반환 요약:
  ...
  - 프로젝트 문서와 관련 project-post를 source context로 쓰라는 지시가 포함됐다.
  - `cruz-lab-knowledge-mcp`는 tool/resource/prompt 경계를 확인하기 위한 작은 로컬 MCP 서버로 다루라는 제약이 포함됐다.
- 판단: 다음 세션에서 `src/content/project-posts/cruz-lab-knowledge-mcp.md` 글감을 잡는 데 쓸 만하다. 과장 방지 문구가 들어 있어 현재 목적과 맞는다.
...
- `get_project_context` with `{ slug: "cruz-lab" }`: 성공. `src/content/projects/cruz-lab.md` 프로젝트 본문과 `cruz-lab-operating-cms`, `cruz-lab-v2-offline-accessibility` project-post 2건이 함께 반환됐다.
...
`src/content/project-posts/cruz-lab-knowledge-mcp.md`를 작성할 때 쓸 수 있는 관찰:

- 이 작업은 블로그/CMS 본체를 바꾸는 작업이 아니라, 기존 Cruz Lab 문서를 read-only MCP 서버로 감싸 에이전트가 조회할 수 있게 한 작은 실험으로 잡는 편이 맞다.
- 실제로 조회된 범위는 프로젝트 문서, 프로젝트 포스트, final post 초안, 이력서 문서다.
- `get_project_context`가 프로젝트 본문과 관련 project-post를 함께 묶어주기 때문에 후속 글이나 면접 질문 준비용 컨텍스트를 만들기 쉽다.
...
- `src/content/project-posts/cruz-lab-knowledge-mcp.md` 파일을 작성할지, 먼저 별도 초안 경로에서 잡을지 결정한다.
